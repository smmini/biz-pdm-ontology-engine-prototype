...
ID_CANDIDATES = ["asset_id", "machineID",
                 "equipment_id", "device_id", "asset", "machine"]
TIME_CANDIDATES = ["observed_at", "datetime", "timestamp", "time", "date", "degradation_started_at",
                   "failure_occurred_at", "maintenance_started_at", "maintenance_completed_at"]
...
def profile_source_file_with_llm(filepath: str, filename: str, df_preview: pd.DataFrame) -> dict:
    ...
    sample_json = df_preview.head(10).to_json(
    ...
    try:
        ...
        logger.warning(
            f"[SourceFamily] LLM profiling failed for '{filename}': {e}. Falling back to rule-based profiling.")
        id_col, time_col = infer_key_signature(all_columns)
        family_id = compute_family_id(id_col, time_col)
...
def build_family_registry(data_dir: str, force_reprofile: bool = False) -> dict:
    ...
    """
    logger.info(
        f"[SourceFamily] Building Stage 0 family registry for data_dir: '{data_dir}' (force_reprofile={force_reprofile})...")
    if not os.path.exists(data_dir):
        logger.warning(
            f"[SourceFamily] Directory '{data_dir}' missing. Returning empty registry.")
        return {}
    ...
    for filename in sorted(os.listdir(data_dir)):
        ...
        if not force_reprofile and existing_meta and "role" in existing_meta and "all_columns" in existing_meta:
            logger.info(
                f"[SourceFamily] Cache Hit for '{filename}' -> keeping Stage 0 metadata.")
            continue

        filepath = os.path.join(data_dir, filename)
        try:
            preview = pd.read_csv(filepath, nrows=10) if ext == ".csv" else pd.read_excel(
                filepath, nrows=10)
            meta = profile_source_file_with_llm(filepath, filename, preview)
            registry[filename] = meta
            updated_count += 1
            logger.info(
                f"[SourceFamily] Stage 0 Profiled '{filename}': role='{meta.get('role')}', cols={len(meta.get('all_columns', []))}, confidence={meta.get('confidence')}")
        except Exception as e:
            logger.warning(
                f"[SourceFamily] Failed to profile '{filename}': {e}")
    ...
    logger.info(
        f"[SourceFamily] Stage 0 Family registry saved to '{FAMILY_REGISTRY_PATH}' with {len(registry)} entries ({updated_count} profiled).")
    return registry
